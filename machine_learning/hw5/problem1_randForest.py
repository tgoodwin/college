...
def main():
	...
	param_grid = { 'n_estimators': [10, 25, 50], 'max_depth': [2, 4, 8, None], 'min_samples_leaf': [1, 2, 4]
	}

	clf = RandomForestClassifier(n_jobs=-1, verbose=1)
	grid_search = GridSearchCV(clf, param_grid, n_jobs=-1, cv=3)

	grid_search.fit(unigram_train, training_labels.ravel())
	print grid_search.best_params_


	print("Final training set score: %f" % grid_search.score(unigram_train, training_labels.ravel()))
	print("Final test set score: %f" % grid_search.score(unigram_test, test_labels.ravel()))
